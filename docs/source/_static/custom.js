document.addEventListener('DOMContentLoaded', function () {
    ...
    const navItemsToHide = ['Getting Started', 'Developer Guide', 'Aligners', 'Use Cases', 'Package Reference'];
    document.querySelectorAll('.nav-links .nav-item').forEach(item => {
        const link = item.querySelector('a');
        if (link && navItemsToHide.includes(link.textContent.trim())) {
        ...
        if (githubLink) {
            ...
            githubLink.setAttribute('target', '_blank');  // 👈 open in new tab
        }
        ...
        if (pypiLink) {
            ...
            pypiLink.setAttribute('target', '_blank');  // 👈 open in new tab
        }

    // Insert About and Release Notes before GitHub/PyPI
    const navBar = document.querySelector('.nav-links');

    function createNavItem(emoji, text, href) {
        const item = document.createElement('div');
        item.className = 'nav-item';

        const link = document.createElement('a');
        link.href = href;
        link.className = 'nav-link external';
        link.textContent = `${emoji} ${text}`;
    ...
    if (navBar) {
        const about = createNavItem('ℹ️', 'About', 'https://github.com/sciknoworg/OntoAligner/blob/main/MAINTANANCE.md#1-objectives--responsibilities');
        const releaseNotes = createNavItem('📝', 'Release Notes', 'https://github.com/sciknoworg/OntoAligner/releases');


        // Insert before first visible GitHub/PyPI link
        const firstExternal = navBar.querySelector('.nav-item .nav-link.external');
        if (firstExternal) {
            navBar.insertBefore(about, firstExternal.parentElement);
            navBar.insertBefore(releaseNotes, firstExternal.parentElement);
        } else {
            // fallback: add to top if no external links found
            navBar.insertBefore(about, navBar.firstChild);
            navBar.insertBefore(releaseNotes, navBar.firstChild);
        }
    }
});
...
function addGithubButton() {
    ...
    // Create a container to hold both logo and GitHub button
    const wrapper = document.createElement("div");
    wrapper.style.display = "flex";
    wrapper.style.alignItems = "center";
    wrapper.style.gap = "12px";  // spacing between logo and button

    // Move the logo link into the wrapper
    logoLink.parentNode.insertBefore(wrapper, logoLink);
    wrapper.appendChild(logoLink);

    // Create the GitHub button container
    const githubDiv = document.createElement("div");
    githubDiv.innerHTML = `<a class="github-button"
                            href="https://github.com/sciknoworg/OntoAligner"
                            data-size="large"
                            data-show-count="true"
                            aria-label="Star sciknoworg/OntoAligner on GitHub"
                            title="OntoAligner on GitHub">
                            Star
                        </a>`;
    // Append GitHub button beside the logo
    wrapper.appendChild(githubDiv);
}
