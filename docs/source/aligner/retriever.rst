...
Reranking refines candidate alignments generated by retrieval models. A retriever first returns
candidate target concepts for each source concept, and a reranking model reorders those candidates
using a stronger relevance model.

Reranking keeps the grouped retrieval format with ``target-cands`` and ``score-cands`` so the
output can still be passed to retrieval postprocessors. When original retrieval scores are available,
they are preserved as ``retriever-score-cands``. Reranking also supports optional score normalization,
allowing raw reranking scores to be kept as-is or transformed with methods such as ``sigmoid``,
``softmax``, or ``minmax`` before selecting the final candidates.
